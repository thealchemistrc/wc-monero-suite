#!/usr/bin/env python3
"""
Test the full pairing flow against the WordPress endpoint.
Requires a pairing session to already exist (created via WP admin panel).

Usage:
  1. Open http://localhost/shop/wp-admin/admin.php?page=wc-xmr-push
  2. Click "Start Pairing" - note the 2 words and pairing ID
  3. Run: python test_pairing.py <pairing_id>
"""

import subprocess, json, sys, hashlib, secrets, base64
...
def compute_sas(server_kx_pk_hex, client_kx_pk_hex):
    """Canonical ordering: sort hex strings, then BLAKE2b."""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python test_pairing.py <pairing_id>")
        print("First create a session via WP admin: Settings -> Monero Push -> Start Pairing")
        sys.exit(1)

    pairing_id = sys.argv[1].strip().lower()
    print("=" * 60)
    print(f"Testing pairing flow for ID: {pairing_id}")
    print("=" * 60)

    # Step 1: GET server's ephemeral public key
    print("\n[1] GET ?pair={} ...".format(pairing_id))
    ...
    server_kx_pk_hex = data['server_kx_pk']
    print(f"    server_kx_pk: {server_kx_pk_hex[:40]}...")

    # Step 2: Device generates its own ECDH keypair
    print("\n[2] Generating device ECDH keypair...")
    ...
    # Step 3: Compute SAS (Short Authentication String)
    print("\n[3] Computing SAS...")
    sas_words = compute_sas(server_kx_pk_hex, client_kx_pk_hex)
    print(f"    SAS: {sas_words[0]} {sas_words[1]}")
    print("    >>> VERIFY these words match what the WordPress admin shows! <<<")

    # Step 4: Derive shared secret and encrypt device's Ed25519 public key
    print("\n[4] Deriving shared secret & encrypting device public key...")
    server_kx_pk = bytes.fromhex(server_kx_pk_hex)
    rx_key, tx_key = nacl.bindings.crypto_kx_client_session_keys(
        client_kx_pk, client_kx_sk, server_kx_pk
    )
    print(f"    rx_key[:20]: {rx_key.hex()[:40]}...")
    ...
    encrypted_b64 = base64.b64encode(nonce + encrypted).decode('ascii')
    print(f"    encrypted_phone_pk (b64): {encrypted_b64[:60]}...")

    # Step 5: POST to server
    print("\n[5] POST encrypted device key to server...")
    ...
    # Step 6: Verify SAS from server response
    if 'sas_words' in data:
        server_sas = data['sas_words']
        print(f"\n[6] Server SAS: {server_sas[0]} {server_sas[1]}")
        print(f"    Client SAS: {sas_words[0]} {sas_words[1]}")
        if server_sas == sas_words:
            print("    [ok] SAS MATCH! Pairing is secure.")
        else:
            print("    [x] SAS MISMATCH! DO NOT CONFIRM.")
    else:
        print("\n[6] No SAS in response (server may need confirmation step)")
