#!/usr/bin/env python3
"""
Simple E2E test: creates a pairing session via WordPress admin-post,
then tests the GET and POST endpoints.

Usage: python test_e2e_simple.py
"""

import subprocess, json, sys, hashlib, secrets, base64, time

WP_URL = "http://localhost/shop"

# BIP39 wordlist (first 2048)
BIP39 = [
...
def main():
    ...
    # Step 1: Create a session via WordPress admin-post
    # This requires a nonce from the admin page, so we need to get it first
    print("\n[1] Getting WordPress admin page for nonce...")
    body, code = run_curl(f'{WP_URL}/wp-admin/admin.php?page=wc-xmr-push')
    print(f"    HTTP {code}")

    if code != '200':
        print("    Cannot access admin page (not logged in).")
        print("    Skipping session creation via admin - testing GET/POST directly.")
        print("    You'll need to create a session manually via the admin panel.")
        print("\n    To test manually:")
        print("    1. Open http://localhost/shop/wp-admin/admin.php?page=wc-xmr-push")
        print("    2. Click 'Start Pairing'")
        print("    3. Note the pairing ID and words")
        print("    4. Run: python test_e2e_simple.py --pairing-id XXXXXX")
        return

    # Extract nonce
    import re
    nonce_match = re.search(r'name="_wpnonce"\s+value="([^"]+)"', body)
    if not nonce_match:
        nonce_match = re.search(r'"_ajax_nonce":"([^"]+)"', body)
    if not nonce_match:
        nonce_match = re.search(r'wpnonce[=:]\s*["\']?([a-f0-9]+)', body)
    
    if nonce_match:
        nonce = nonce_match.group(1)
        print(f"    Found nonce: {nonce[:20]}...")
        
        # Try to create a session
        print("\n[2] Creating pairing session via admin-post...")
        body2, code2 = run_curl(
            '-X', 'POST',
            '-d', f'_wpnonce={nonce}&action=wc_xmr_push_start_pairing',
            '-c', 'cookies.txt',
            '-b', 'cookies.txt',
            f'{WP_URL}/wp-admin/admin-post.php'
        )
        print(f"    HTTP {code2}: {body2[:200]}")
    else:
        print("    Could not find nonce in admin page.")
        print("    You may need to log in first.")
